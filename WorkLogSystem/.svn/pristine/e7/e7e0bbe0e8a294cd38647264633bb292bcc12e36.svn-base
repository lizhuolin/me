	     ...
	     <!-- 第二步：写自己查询需要的数据  需要提交的查询表单数据 start -->
	      ...
	      <div class="con_top">
	        <div class="con_search">
	          <span>时间：
		          <s:select list="#yearList" id="year" name="year"/> 年
		          <s:select list="#monthList" id="month" name="month" />月
	          </span>
	        </div>
	        <!--  需要提交的查询表单数据 end -->
...
<%--             <td><input type="checkbox" value="<s:property value="timereportId"/>"></td>
 --%>            <s:if test="#session.USER.isUserPlatform">
           ...
           <td align="center"><s:date name="projectStartTime"/></td>
           <td align="center"><s:date name="projectEndTime"/></td>
           <td align="center"><s:date name="projectRealStartTime"/></td>
           <td align="center"><s:date name="projectRealEndTime"/></td>
