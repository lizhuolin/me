...
public class ExcelUtil {
		...
		try {
			...
			if(timeReports != null){
				for(int j = 0; j < timeReports.size(); j++){
					...
					if(timeReports.get(j).getProjectStatus().equals("0")){
						cell19.setCellValue("未开始");
					}else if(timeReports.get(j).getProjectStatus().equals("1")){
							cell19.setCellValue("进行中");
					}
					else if(timeReports.get(j).getProjectStatus().equals("2")){
						cell19.setCellValue("已结束");
					}else{
						cell19.setCellValue("");
					}
	...
	public static void exportPlanExcel(List<TblPlan> planList, ServletOutputStream outputStream) {
		try {
			...
			// 3.2、创建列标题行；并且设置列标题
			HSSFRow row2 = sheet.createRow(1);
			String[] titles = { "计划名称", "计划负责人", "计划预计开始时间", "计划预计结束时间", "计划实际开始时间", "计划实际结束时间", "计划目标", "计划进度", "计划状态",
					"计划内容", "计划等级", "计划工时", "所属子项目", "计划备注" };
			for (int i = 0; i < titles.length; i++) {
				HSSFCell cell2 = row2.createCell(i);
				// 加载单元格样式
				cell2.setCellStyle(style2);
