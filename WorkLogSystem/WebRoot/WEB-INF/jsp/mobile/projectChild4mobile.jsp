...
<div id="header_wrapper">
    <div class="title">
    	<div class="title_button"><a href="<%=basePath%>mobile/project/mobile_projectDetail4mobile.action?tblProject.projectId=<s:property value="tblProject.projectId"/>"><img src="<%=basePath%>images/back.png"></a></div>
 		 <div class="title_text">子项目详细信息</div>
  </div>
  <div class="explain">
  		<div class="explain_name">
        <div class="dt_dian">●<span ><s:property value="tblProjectChild.projectName" /></span></div>
        </div>
        <div class="explain_content"> 
         <p>负责人:<span><s:property value="tblProjectChild.responsible.name" /></span></p>
		 <p>子项目状态:<span><s:if test="tblProjectChild.projectChildStatus==0" >未开始</s:if><s:elseif test="tblProjectChild.projectChildStatus==1">进行中</s:elseif><s:else>已结束</s:else></span></p>
         <p>创建时间:<span><s:property value="tblProjectChild.createTime" /></span></p>
         <p>项目目标:<span><s:property value="tblProjectChild.projectChildTarget" /></span></p>
         <p>子项目计划工时:<span><s:property value="tblProjectChild.planTime"/></span></p>
         <p>子项目等级:<span><s:if test="tblProjectChild.projectLevel==0" >低</s:if><s:elseif test="tblProjectChild.projectLevel==1">一般</s:elseif><s:else>高</s:else></span></p>
		 <p>子项目进度:<span><s:property value="tblProjectChild.projectProgress"/>%</span></p>
         <p>计划开始－结束时间:</p><p><span><s:date name="tblProjectChild.projectChildStarttime" format="yyyy-MM-dd"/>—<s:date name="tblProjectChild.projectChildEndtime" format="yyyy-MM-dd"/></span></p>
         <p>实际开始－结束时间:</p><p><span><s:date name="tblProjectChild.projectChildRealstarttime" format="yyyy-MM-dd"/>—<s:date name="tblProjectChild.projectChildRealendtime" format="yyyy-MM-dd"/></span></p>
    	</div>
        </div>
      ...
      <section>
           ...
           <div class="xuhao"><s:property value="#s.index"/></div><input type="text"  value="<s:property value="planName"/>"  readonly/>
            ...
            <div class="dt_other_left">
                ...
                <p>计划预计开始－预计结束时间：</p><p><span><s:date name="planStartTime" format="yyyy-MM-dd"/>—<s:date name="planEndTime" format="yyyy-MM-dd"/></span></p>
         		</div>
