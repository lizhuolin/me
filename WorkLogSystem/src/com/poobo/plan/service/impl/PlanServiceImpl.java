...
public class PlanServiceImpl extends BaseServiceImpl<TblPlan> implements IPlanService{
	...
	public PageController <TblPlan> getPlanListByProjectId(TblPlan tblPlan,String projectId, int pageNow,
		...
		if(tblPlan!=null){
			if (StringUtils.isNotEmpty(tblPlan.getPlanName())) {
				if(tblPlan.getPlanName().equals("%")){
					namedParam.put("planName", "%"+"\\%"+"%");
					sb.append(" and planName like :planName");
				}else{
					namedParam.put("planName", "%"+tblPlan.getPlanName()+"%");
					sb.append(" and planName like :planName");
				}
			}if (tblPlan.getPlanManager()!=null&&StringUtils.isNotEmpty(tblPlan.getPlanManager().getName())){
				if(tblPlan.getPlanManager().getName().equals("%")){
	...
	@Override
	public PageController<TblPlan> getPlanListByProjectChildId(
			TblPlan tblPlan,String projectChildRealId,int pageNow,int pageSize) {
		Criteria criteria=planDao.createCriteria(TblPlan.class);
		if(tblPlan!=null){
			if(tblPlan.getPlanName().equals("%")){
				criteria.add(Restrictions.ilike("planName", "\\%"));
			}else if(StringUtils.isNotEmpty(tblPlan.getPlanName())){
				criteria.add(Restrictions.ilike("planName", tblPlan.getPlanName()));
			}
			
		}
		criteria.add(Restrictions.eq("tblProjectChild.projectChildRealId",projectChildRealId));
