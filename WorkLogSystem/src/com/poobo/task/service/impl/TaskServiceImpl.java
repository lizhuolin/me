...
public class TaskServiceImpl extends BaseServiceImpl<TblTask> implements ITaskService{
	...
	public PageController<TblTask> findTaskList(TblTask tblTask, int pageNow, int pageSize) {
		...
		if (null != tblTask) {
			...
			if(GetSessionData.getDataPrivilege().equals(EnumDataPrivilege.SEESELF.getValue())){
				sb.append(" and taskResponsible.userId=:tblTask");
				namedParam.put("tblTask", GetSessionData.getUser().getUserId());
			}
	...
	public List<TblTask> findTaskList4Mobile(TblTask tblTask, int rowIndex, int pageSize) {
		...
		if(null!=tblTask){
			...
			//数据权限
			if(GetSessionData.getDataPrivilege().equals(EnumDataPrivilege.SEESELF.getValue())){
				criteria.add(Restrictions.eq("taskResponsible.userId", GetSessionData.getUser().getUserId()));
			}
			else if(tblTask.getTblCompany() != null){
